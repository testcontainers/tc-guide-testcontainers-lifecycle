...
date: 2023-03-04T09:39:58+05:30
draft: false
...
* We have explicitly stopped the Postgres container in the *@AfterAll* callback method. But even if we don't explicitly stop the container, Testcontainers library takes care of stopping and removing the containers by using https://github.com/testcontainers/moby-ryuk[Ryuk Container] behind the scenes.
...
Tip: Instead of starting the containers sequentially, we can start the containers in parallel by using *Startables.deepStart(postgres, kafka).join();*
